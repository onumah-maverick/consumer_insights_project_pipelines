...
class RecruitmentDownload:
    ...
    def transform_recruitment_profile(subj_id, survey_id, api_key, username, password):
        ...
        try:
            ...
            variables = ['surveyor', 'gps_point', 'region', 'name', 'phone_number',
                        'age', 'survey_agree', 'job_type','market_research_study', 
                        'own_motorbike', 'consider_bike','usage_history', 'bike_frequency', 
                        'bike_purpose']
            ...
            audit_items_df[cols] = audit_items_df['gps_point'].str.extract(pattern, expand=True)
            audit_items_df.drop(columns=["gps_point"], inplace=True) # Remove original GPS column
...
# ------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def transform_media_habits(subj_id, survey_id, api_key, username, password):
        ...
        try:
            ...
            # List of FullVariable values to extract
            variables = ['media_source', 'tv_channels', 'tv_channel_prefer', 'radio_stations', 'radio_station_prefer',
                         'social_media', 'social_media_prefer', 'social_media_hours']
            ...
            # Explode dataframe
            exploded_data = backbone.explode_columns(new_combined_df, ['media_source', 'tv_channels', 'radio_stations', 'social_media', ], ",")
...
# ------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def transform_brand_awareness(subj_id, survey_id, api_key, username, password):
        ...
        try:
            ...
            # List of FullVariable values to extract
            variables = ['tom', 'spon', 'score_card', 'brand_engage_12', 'brand_engage_3',
                          'brand_engage_1', 'brand_engage_7', 'current_use', 'bike_number',
                         'brand_prefer', 'future_brand', 'brand_prefer_reasons',
                         'brand_prefer_likes', 'brand_prefer_dislikes', 'previous_bike', 
                         'previous_bike_brand', 'bike_switch_reason', 'bike_selection_criteria']
            ...
            # List of FullVariable values to extract
            variables = ['Brand1', 'Brand2', 'Brand3', 'Brand4', 'Brand5',
            ...
            # Explode dataframe
            exploded_data = backbone.explode_columns(new_combined_df, ['spon', 'score_card', 'brand_engage_12', 'brand_engage_3', 
                                                                       'brand_engage_1', 'brand_engage_7','future_brand', 
                                                                       'brand_prefer_reasons', 'brand_prefer_likes', 'brand_prefer_dislikes',
                                                                       'previous_bike', 'bike_selection_criteria'], ",")
...
# ------------------------------------------------------------------------------------------------------------------------------------------
    @staticmethod
    def brand_description(subj_id, survey_id, api_key, username, password):
        """
        ...
        try:
            ...
            # List of FullVariable values to extract
            variables = ['Boxer.', 'Yamaha.', 'Honda.', 'TVS.','Apsonic.', 'Haojue.', 'Royal / Royal Motors.',
                         'Sanya.', 'Suzuki.', ]
            ...
            for i in range(len(df_items_use)):
                ...
                if row['Element'] == 'Text' and row['Value'] in variables:
                    ...
                    # If we hit a new item_category and it's not the first item, save the previous item
                    if var_name == "Young and upcoming runner":
                        if not first_item:
                            all_items.append(current_item)
                            current_item = {}
                        else:
            ...
            return final_old_items#exploded_data    


# ------------------------------------------------------------------------------------------------------------------------------------------
# ------------------------------------------------------------------------------------------------------------------------------------------
    @staticmethod
    def brand_association(subj_id, survey_id, api_key, username, password):
        """
        This function creates and stores the data expected for new items in a dataframe.
        The variables that define new items are selected and manipulated in order to arrive
        at a desired dataframe format. Responses from the FullVariable, QuestionAnswer sections
        ...
        try:
            ...
            for i in range(len(df_items_use)):
                ...
                if row['Element'] == 'Text' and row['Value'] in variables:
                    ...
                    # If we hit a new item_category and it's not the first item, save the previous item
                    if var_name == "Is a brand I trust":
                        if not first_item:
                            all_items.append(current_item)
                            current_item = {}
                        else:
...
# ------------------------------------------------------------------------------------------------------------------------------------------
    @staticmethod
    def advert_general(subj_id, survey_id, api_key, username, password):
        """
        Extract the values you need
        The parsed xml data is grouped into chapters. The target chapters list is used to select the desired chapters.
        The chapters are stored in a dataframe from which the necessary variables are selected.
        ...
        try:
            ...
            # List of FullVariable values to extract
            variables = ['brand_advert','boxer_bike_advert','boxer_advert_type', 
                         'boxer_advert_location', 'boxer_rating', 'boxer_recommend',]
            ...
            # Explode dataframe
            exploded_data = backbone.explode_columns(new_combined_df, ['brand_advert','boxer_advert_location'], ",")
...
# ------------------------------------------------------------------------------------------------------------------------------------------
    @staticmethod
    def transform_advert_impression(subj_id, survey_id, api_key, username, password):
        """
        This function creates and stores the data expected for new items in a dataframe.
        The variables that define new items are selected and manipulated in order to arrive
        at a desired dataframe format. Responses from the FullVariable, QuestionAnswer sections
