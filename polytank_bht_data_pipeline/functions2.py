...
class RecruitmentDownload:
    ...
    def transform_recruitment_profile(subj_id, survey_id, api_key, username, password):
        ...
        try:
            ...
            variables = ['surveyor', 'gps', 'region', 'age_range', 'gender',
                        'Contact', 'employment_status', 'job_type', 'survey_recency',]
...
# ------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def transform_social_profile(subj_id, survey_id, api_key, username, password):
        ...
        try:
            ...
            # List of FullVariable values to extract
            variables = ['participate', 'own_tank', 'consider_tank', 'segment', 
                         'business_type', 'tank_purpose', 'decision_maker', 'tank_size', 
                         'tank_model',]
            ...
            # Explode dataframe
            exploded_data = backbone.explode_columns(new_combined_df, ['tank_purpose',], ",")
...
# ------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def transform_media_habits(subj_id, survey_id, api_key, username, password):
        ...
        try:
            ...
            # List of FullVariable values to extract
            variables = ['channel_for_reach', 'radio_stations', 'tv_stations']
            ...
            # List of FullVariable values to extract
            variables = ['TV','Radio', 'WhatsApp','Facebook','TikTok', 'YouTube', 'Billboards / outdoor ads', 'Print newspapers',]
            ...
            # Explode dataframe
            exploded_data = backbone.explode_columns(new_combined_df, ['radio_stations', 'tv_stations'], ",")
...
# ------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def transform_brand_awareness(subj_id, survey_id, api_key, username, password):
        ...
        try:
            ...
            # List of FullVariable values to extract
            variables = ['tom_aware', 'extra_aware', 'extra_aware_other', 'advert_aware', 'advert_aware_other',
                          'confirm_brand_aware', 'other_adverts', 'recently_used_brand', 'brand_purchase_duration',
                         'consider_main_brand', 'preferred_brand', 'awareness_location','channel_influencial',]
            ...
            #---------------------------------------------------------------------------------------------------------------------
            new_profile_extra = df.loc[(df['Element'] == 'FullVariable') | (df['Element'] == 'Text') | (df['Element'] == 'TopicAnswer')]
            new_profile_extra = new_profile_extra.reset_index(drop=True)
            start_indices = new_profile_extra.index[(new_profile_extra['Element'] == 'FullVariable') & (new_profile_extra['Value'] == 'brand_adverts')]
            end_indices = new_profile_extra.index[(new_profile_extra['Element'] == 'FullVariable') & (new_profile_extra['Value'] == 'other_adverts')]
            df_profile_extra = new_profile_extra.iloc[start_indices[0]:end_indices[0]]

            # List of FullVariable values to extract
            variables = ['Polytank','Sintex','Duraplast','Mountain','Polystar',]
            ...
            # Explode dataframe
            exploded_data = backbone.explode_columns(new_combined_df, ['other_adverts', 'recently_used_brand', 'confirm_brand_aware',\
                                                                       'extra_aware','brand_purchase_duration', 'awareness_location'], ",")
            
        except TypeError:
            print("Desired values not available")
        else:
            return exploded_data#new_combined_df
# ------------------------------------------------------------------------------------------------------------------------------------------
# ------------------------------------------------------------------------------------------------------------------------------------------
    @staticmethod
    def transform_tank_preference(subj_id, survey_id, api_key, username, password):
        """
        Extract the values you need
        The parsed xml data is grouped into chapters. The target chapters list is used to select the desired chapters.
        The chapters are stored in a dataframe from which the necessary variables are selected.
        ...
        try:
            ...
            # List of FullVariable values to extract
            variables = ['own_tank','purchase_point','overall_best', 'overall_best_reason',
                         'reason_for_tank', 'satisfaction_level', 'recommendation_likelihood', 
                         'next_brand', 'first_choice', 'first_choice_likelihood']
            ...
            # Explode dataframe
            exploded_data = backbone.explode_columns(new_combined_df, ['reason_for_tank','purchase_point'], ",")
...
# ------------------------------------------------------------------------------------------------------------------------------------------
    @staticmethod
    def transform_tank_impression(subj_id, survey_id, api_key, username, password):
        """
        ...
        try:
            ...
            # List of FullVariable values to extract
            variables = ["This brand’s tanks are strong and durable.",'They last a very long time without damage.','The tanks are good quality for the price.',\
                         "They are built to withstand Ghana’s weather conditions.",'The brand offers different sizes to meet my needs.','The tank materials are safe for storing drinking water.',\
                         'The installation process is easy and hassle-free.', 'This brand has good warranty and after-sales support.', 'The tanks are less likely to leak or crack.'\
                         'They have wide availability in shops near me.', "A brand I can trust for my family’s well-being.", 'It feels modern and innovative.',\
                         'It is a brand that keeps its promises.', 'The brand offers great value for money.', 'The design of the tank is attractive.',\
                         "The brand’s logo and markings make it easy to identify.", 'I can identify the brand easily from a distance', 'This brand is better than most alternatives.'\
                         'Brand I trust', 
                        ]
            ...
            for i in range(len(df_items_use)):
                ...
                if row['Element'] == 'Text' and row['Value'] in variables:
                    ...
                    # If we hit a new item_category and it's not the first item, save the previous item
                    if var_name == "This brand’s tanks are strong and durable.":
                        if not first_item:
                            all_items.append(current_item)
                            current_item = {}
                        else:
            ...
            #---------------------------------------------------------------------------------------------------------------------
            indices = df.index[(df['Element'] == 'Variable') & (df['Value'] == 'brand_impression')]
            end = df.index[(df['Element'] == 'Variable') & (df['Value'] == 'overall_best')]
            df_profile_extra = df.iloc[indices[0]:end[0]].reset_index(drop=True)

            all_items_2 = []
            brand_impression_values = []

            for i in range(len(df_profile_extra)):
                row = df_profile_extra.iloc[i]
                
                # Look for Variable row with 'brand_impression'
                if row['Element'] == 'Variable' and row['Value'] == 'brand_impression':
                    # Get the corresponding FullVariable value from next row
                    if i + 1 < len(df_profile_extra) and df_profile_extra.iloc[i + 1]['Element'] == 'FullVariable':
                        var_value = df_profile_extra.iloc[i + 1]['Value']
                        brand_impression_values.append(var_value)
                    
                    # Skip the FullVariable row
                    i += 1

            # Create final dictionary with all values collected
            all_items_2.append({'brand_impression': ", ".join(brand_impression_values)})
            profile_df = pd.DataFrame(all_items_2)
            ...
            #---------------------------------------------------------------------------------------------------------------------
            final_old_items = pd.concat([df_pivoted, profile_df, audit_items_df],axis=1)
            ...
            # Explode dataframe
            exploded_data = backbone.explode_columns(final_old_items, ['brand_impression'], ",")            
        except TypeError:
            print("Desired values not available")
        else:
            return exploded_data    
