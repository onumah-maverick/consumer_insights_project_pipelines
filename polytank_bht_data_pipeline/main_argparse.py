...
def main():   
    """
        ...
        2. "tank_social"       | respondent overview of tank
        3. "tank_preference"   | favourite tank and reasons
        4. "brand_aware"       | tank brand knowledge
        5. "media"             | social media usage
        6. "brand_impress"     | perception of brands
        ...
        for found_date in date_list_formatted:
            try:
                ...
                for outlet in new_list: # iterate through ids, transform their data and append them to a big list
                    dataframe_return =  recruit.transform_tank_impression(outlet, recruit_survey_id, api_key, username, password) # not finished
                ...
                # merged_df.to_excel(f'brand_impression_{args.start}_{args.end}.xlsx', index=False)
                merged_df.to_sql(con=my_conn, name='owner_impression', if_exists='append', index=False) # consider appending/replacing in some cases
                print("="*40)
    ...
    ## ------------------------------------------------------------------------------------------------------------------------------------------
        ...
        for found_date in date_list_formatted:
            try:
                ...
                merged_df.to_sql(con=my_conn, name='brand_awareness', if_exists='append', index=False) # consider appending/replacing in some cases
                print("="*40)
                print("Finished process")
            except ValueError:
                print("No subject ids for the day!")
                print("="*40) 
    ## ------------------------------------------------------------------------------------------------------------------------------------------
    elif args.file == "tank_preference":
        ...
        for found_date in date_list_formatted:
            try:
                ...
                new_list = general.get_store_id(api_key, username, password, recruit_survey_id, found_date) # get store ids for given period, returns a list of store ids
                for outlet in new_list: # iterate through ids, transform their data and append them to a big list
                    dataframe_return = recruit.transform_tank_preference(outlet, recruit_survey_id, api_key, username, password)
                    new_final_df.append(dataframe_return)           
                # Save processed stores data
                merged_df = pd.concat(new_final_df, axis=0)
                merged_df.insert(0, 'Period', dt.datetime.today().replace(day=1).date().strftime('%Y-%m-%d')) # Add 'period' column
                # merged_df.to_excel(f'tank_preference_{args.start}_{args.end}.xlsx', index=False)
                merged_df.to_sql(con=my_conn, name='tank_preferences', if_exists='append', index=False) # consider appending/replacing in some cases
                print("="*40)
                print("Finished process")
            except ValueError:
                print("No subject ids for the day!")
    ...
    ## ------------------------------------------------------------------------------------------------------------------------------------------
        ...
        for found_date in date_list_formatted:
            try:
                ...
                merged_df = pd.concat(new_final_df, axis=0)
                merged_df.insert(0, 'Period', dt.datetime.today().replace(day=1).date().strftime('%Y-%m-%d')) # Add 'period' column
                # merged_df.to_excel(f'tank_social_{args.start}_{args.end}.xlsx', index=False)
                merged_df.to_sql(con=my_conn, name='owner_overview', if_exists='append', index=False) # consider appending/replacing in some cases
                print("="*40)
                print("Finished process")
            except ValueError:
                print("No subject ids for the day!")
