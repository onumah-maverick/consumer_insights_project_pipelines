...
class RecruitmentDownload:
    ...
    def transform_recruitment_profile(subj_id, survey_id, api_key, username, password):
        ...
        try:
            ...
            variables = ['Enumerator_Name', 'Region', 'Store_Name', 'Outlet_Code', 'Visit_Date', 'Contact', 
                        'gps_address', ]
...
# ------------------------------------------------------------------------------------------------------------------------------------------
    @staticmethod
    def transform_outlet_description(subj_id, survey_id, api_key, username, password):
        """
        Extract the values you need
        The parsed xml data is grouped into chapters. The target chapters list is used to select the desired chapters.
        The chapters are stored in a dataframe from which the necessary variables are selected.
        ...
        try:
            ...
            # List of FullVariable values to extract
            variables = ['branded_status', 'brand_type', 'vc_status', 'vc_count',
                        'abl_fridge_status', 'abl_fridge_count']
            ...
            # List of FullVariable values to extract
            variables = ['vc_Brand1', 'vc_Brand2', 'vc_Brand3', 'vc_Brand4', 'vc_Brand5',
                         'vc_Brand6', 'vc_Brand7', 'vc_Brand8', 'vc_Brand9', 'vc_Brand10',]
            ...
            # Explode dataframe
            exploded_data = backbone.explode_columns(new_combined_df, ['brand_type',], ",")
...
# ------------------------------------------------------------------------------------------------------------------------------------------
    @staticmethod
    def transform_product_availability(subj_id, survey_id, api_key, username, password):
        """
        Extract the values you need
        The parsed xml data is grouped into chapters. The target chapters list is used to select the desired chapters.
        The chapters are stored in a dataframe from which the necessary variables are selected.
        ...
        try:
            ...
            # List of FullVariable values to extract
            variables = ['CLUB DRAUGHT 500ML_Visit_Availability', 'CLUB 625ML RB_Visit_Availability',
                         'CLUB 330ML RB_Visit_Availability', 'EAGLE LAGER 375ML_Visit_Availability',
                         'EAGLE STOUT 375ML_Visit_Availability', 'BETA MALT 330ML RG_Visit_Availability',
                         'BETA MALT 330ML PET_Visit_Availability', 'BRUTAL FRUIT 275ML NRB_Visit_Availability',
                         'SHANDY 500ML RB_Visit_Availability', 'BRUTAL FRUIT 500ML CAN_Visit_Availability',
                         'BUDWEISER 330ML BOTTLE_Visit_Availability', 'CORONA EXTRA 330ML BOTTLE_Visit_Availability',
                         'STELLA ARTOIS 330ML BOTTLE_Visit_Availability',]
            ...
            for i in range(len(df_items_use)):
                ...
                if row['Element'] == 'FullVariable' and row['Value'] in variables:
                    ...
                    # If we hit a new item_category and it's not the first item, save the previous item
                    if var_name == 'CLUB DRAUGHT 500ML_Visit_Availability':
                        if not first_item:
                            all_items.append(current_item)
                            current_item = {}
                        else:
...
# ------------------------------------------------------------------------------------------------------------------------------------------
    @staticmethod
    def transform_product_price(subj_id, survey_id, api_key, username, password):
        """
        Extract the values you need
        The parsed xml data is grouped into chapters. The target chapters list is used to select the desired chapters.
        The chapters are stored in a dataframe from which the necessary variables are selected.
        ...
        try:
            ...
            # List of FullVariable values to extract
            variables = ['CLUB DRAUGHT 500ML_Price', 'CLUB 625ML RB_Price', 'CLUB 330ML RB_Price', 
                         'EAGLE LAGER 375ML_Price', 'EAGLE STOUT 375ML_Price', 'BETA MALT 330ML RG_Price',
                         'BETA MALT 330ML PET_Price', 'BRUTAL FRUIT 275ML NRB_Price', 'SHANDY 500ML RB_Price', 
                         'BRUTAL FRUIT 500ML CAN_Price', 'BUDWEISER 330ML BOTTLE_Price', 'CORONA EXTRA 330ML BOTTLE_Price',
                         'STELLA ARTOIS 330ML BOTTLE_Price',]
            ...
            for i in range(len(df_items_use)):
                ...
                if row['Element'] == 'FullVariable' and row['Value'] in variables:
                    ...
                    # If we hit a new item_category and it's not the first item, save the previous item
                    if var_name == 'CLUB DRAUGHT 500ML_Price':
                        if not first_item:
                            all_items.append(current_item)
                            current_item = {}
                        else:
...
# ------------------------------------------------------------------------------------------------------------------------------------------
    @staticmethod
    def transform_product_status(subj_id, survey_id, api_key, username, password):
        """
        Extract the values you need
        The parsed xml data is grouped into chapters. The target chapters list is used to select the desired chapters.
        The chapters are stored in a dataframe from which the necessary variables are selected.
        ...
        try:
            ...
            # List of FullVariable values to extract
            variables = ['CLUB DRAUGHT 500ML_In_Stock_Status', 'CLUB 625ML RB_In_Stock_Status', 'CLUB 330ML RB_In_Stock_Status', 
                         'EAGLE LAGER 375ML_In_Stock_Status', 'EAGLE STOUT 375ML_In_Stock_Status', 'BETA MALT 330ML RG_In_Stock_Status',
                         'BETA MALT 330ML PET_In_Stock_Status', 'BRUTAL FRUIT 275ML NRB_In_Stock_Status', 'SHANDY 500ML RB_In_Stock_Status', 
                         'BRUTAL FRUIT 500ML CAN_In_Stock_Status', 'BUDWEISER 330ML BOTTLE_In_Stock_Status', 'CORONA EXTRA 330ML BOTTLE_In_Stock_Status',
                         'STELLA ARTOIS 330ML BOTTLE_In_Stock_Status',]
            ...
            for i in range(len(df_items_use)):
                ...
                if row['Element'] == 'FullVariable' and row['Value'] in variables:
                    ...
                    # If we hit a new item_category and it's not the first item, save the previous item
                    if var_name == 'CLUB DRAUGHT 500ML_In_Stock_Status':
                        if not first_item:
                            all_items.append(current_item)
                            current_item = {}
                        else:
