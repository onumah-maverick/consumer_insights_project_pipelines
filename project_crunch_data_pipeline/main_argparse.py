...
def main():   
    """
        ...
        2. "social"            | respondent social classification
        3. "biscuit_overview"  | biscuit consumption
        4. "brand_aware"       | biscuit brand knowledge
        5. "media"             | social media usage
        6. "advert"            | advert response
        7. "filter_sku"        | selected skus
        ...
        py main_argparse.py -f recruit_profile -s 2025-10-17 -e 2025-10-18

        The above code downloads recruit profile for the periods 17th October 2025
        ...
        for found_date in date_list_formatted:
            try:
                ...
                for outlet in new_list: # iterate through ids, transform their data and append them to a big list
                    dataframe_return =  recruit.transform_sku_filter(outlet, recruit_survey_id, api_key, username, password) # not finished
                ...
                # merged_df.to_excel(f'filter_sku_{args.start}_{args.end}.xlsx', index=False)
                merged_df.to_sql(con=my_conn, name='special_skus', if_exists='append', index=False) # consider appending/replacing in some cases
                print("="*40)
    ...
    # # --------------------------------------------------------------------------------------------------------------------------------
        ...
        for found_date in date_list_formatted:
            try:
                ...
                for outlet in new_list: # iterate through ids, transform their data and append them to a big list
                    dataframe_return = recruit.transform_advert(outlet, recruit_survey_id, api_key, username, password)
                    new_final_df.append(dataframe_return)     
                # Save processed stores data
                merged_df = pd.concat(new_final_df, axis=0)
                merged_df.insert(0, 'Period', dt.datetime.today().replace(day=1).date().strftime('%Y-%m-%d')) # Add 'period' column
        ...
        for found_date in date_list_formatted:
            try:
                ...
                for count, outlet in enumerate(new_list):
                    print(f"Load: {count}",end="\n\n")
                    dataframes_dict = recruit.transform_brand_awareness(outlet, recruit_survey_id, api_key, username, password)
                    # print(dataframes_dict)
                    for table_name, df in dataframes_dict.items():
                        table_name_lower = table_name.lower()
                        df.to_sql(con=my_conn, name=table_name_lower, if_exists='append', index=False, chunksize=100)
                print("="*40)
                print("Finished process")
                ...
                print("="*40) 
    ## ------------------------------------------------------------------------------------------------------------------------------------------
    elif args.file == "biscuit_overview":
        ...
        for found_date in date_list_formatted:
            try:
                ...
                new_list = general.get_store_id(api_key, username, password, recruit_survey_id, found_date) # get store ids for given period, returns a list of store ids
                for outlet in new_list: # iterate through ids, transform their data and append them to a big list
                    dataframe_return = recruit.transform_biscuit_overview(outlet, recruit_survey_id, api_key, username, password)
                    new_final_df.append(dataframe_return)           
                # Save processed stores data
                merged_df = pd.concat(new_final_df, axis=0)
                merged_df.insert(0, 'Period', dt.datetime.today().replace(day=1).date().strftime('%Y-%m-%d')) # Add 'period' column
                # merged_df.to_excel(f'biscuit_overview_{args.start}_{args.end}.xlsx', index=False)
                merged_df.to_sql(con=my_conn, name='overview', if_exists='append', index=False) # consider appending/replacing in some cases
                print("="*40)
                print("Finished process")
            except ValueError:
                print("No subject ids for the day!")
    ...
    ## ------------------------------------------------------------------------------------------------------------------------------------------
        ...
        for found_date in date_list_formatted:
            try:
                ...
                new_list = general.get_store_id(api_key, username, password, recruit_survey_id, found_date) # get store ids for given period, returns a list of store ids
                for outlet in new_list: # iterate through ids, transform their data and append them to a big list
                    dataframe_return = recruit.transform_social_profile(outlet, recruit_survey_id, api_key, username, password)
                    new_final_df.append(dataframe_return)           
                # Save processed stores data
                merged_df = pd.concat(new_final_df, axis=0)
                merged_df.insert(0, 'Period', dt.datetime.today().replace(day=1).date().strftime('%Y-%m-%d')) # Add 'period' column
                # merged_df.to_excel(f'social_{args.start}_{args.end}.xlsx', index=False)
                merged_df.to_sql(con=my_conn, name='social', if_exists='append', index=False) # consider appending/replacing in some cases
                print("="*40)
                print("Finished process")
            except ValueError:
                print("No subject ids for the day!")
